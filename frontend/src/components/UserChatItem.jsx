...
const UserChatItem = ({ user, currentUser, isSelected, onUserSelect, onPinChange, onDeleteConversation, isOnline = false, hasUnreadMessage = false, pinnedUsers = [] }) => {
  ...
  useEffect(() => {
    checkPinStatus();
  }, [user.username, currentUser, pinnedUsers]);
  ...
  const handlePinToggle = async (e) => {
    ...
    try {
      ...
      logger.debug('Attempting to pin/unpin user', {
      ...
      <div className="d-flex align-items-center gap-3">
        ...
        <div>
          <div className="fw-medium text-dark d-flex align-items-center gap-2">
            ...
            {hasUnreadMessage && (
              <div className="bg-primary rounded-circle" style={{ width: "8px", height: "8px" }} title="New message"></div>
            )}
          </div>
